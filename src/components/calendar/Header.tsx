...
export default function Header({ viewStartDate, setViewStartDate, onToday, daysCount, setDaysCount }: HeaderProps) {
  ...
  return (
    <div className="h-14 flex items-center justify-between px-4 bg-[#f0f2f5] border-b">
      <div className="flex items-center gap-4">
        {/* Date Navigator */}
        <div className="flex items-center gap-1.5 font-bold">
          <button 
            onClick={() => setViewStartDate(subDays(viewStartDate, 7))}
            className="p-1 bg-gray-200 hover:bg-gray-300 rounded transition-colors group"
          >
            <ChevronLeft size={14} />
          </button>
        ...
        <button 
          ...
          className="text-xs font-bold px-3 py-1 border border-gray-400 rounded hover:bg-gray-200 active:scale-95 transition-transform text-gray-700 uppercase"
        >
          today
        </button>
            ...
            return (
              <button
                ...
                className={`
                  ...
                  ${isCurrent ? 'bg-green-600 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-200'}
                `}
              >
                {m}
              </button>
      ...
      {/* Right side: Options & Print */}
      <div className="hidden sm:flex items-center gap-3">
        <select 
          value={daysCount}
          onChange={(e) => setDaysCount(Number(e.target.value))}
          className="text-xs font-bold border rounded-lg px-2 py-1.5 bg-white shadow-sm"
        >
        ...
        <button className="p-2 hover:bg-gray-100 rounded-lg text-gray-500" title="Print Calendar">
          <Printer size={18} />
        </button>
      </div>
    </div>
